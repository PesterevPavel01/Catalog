...
public sealed class RemoveOrderItemDomainEventHandler : INotificationHandler<RemoveOrderItemDomainEvent>
{
    private readonly IBus _bus;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ITelegramService _telegramService;

    public RemoveOrderItemDomainEventHandler(IBus bus, IUnitOfWork unitOfWork,
    ...
    public async Task Handle(RemoveOrderItemDomainEvent orderProducedDomainEvent, CancellationToken cancellationToken)
    {
        var order = await _unitOfWork.GetRepository<Order>()
            .GetFirstOrDefaultAsync(
                predicate: x => x.Id == orderProducedDomainEvent.OrderId,
        ...
        if (order.Status == OrderStatus.PendingApproval)
            await _bus.Publish(new RemoveOrderItemEvent(order.ConvertToDto()));
    }
}