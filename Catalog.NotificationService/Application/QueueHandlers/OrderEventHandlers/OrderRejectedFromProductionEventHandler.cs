...
public sealed class OrderRejectedFromProductionEventHandler : IHandleMessages<OrderRejectedFromProductionEvent>
{
    ...
    public OrderRejectedFromProductionEventHandler(ILogger<OrderRejectedEventHandler> logger, ITelegramService telegramService,
    ...
    public async Task Handle(OrderRejectedFromProductionEvent message)
    {
        ...
        await _telegramService.SendMessageAsync($"ЗАКАЗ НЕ ПРИНЯТ В ПРОИЗВОДСТВО: заказ: \"{message.Order.Title}\" пользователя: \"{message.Order.User}\"!");
