...
namespace Catalog.NotificationService.Application.QueueHandlers.OrderEventHandlers
{
    public class OrderExportedEventHandler : IHandleMessages<OrderExportedEvent>
    {
        ...
        public OrderExportedEventHandler(ITelegramService telegramService,
        ...
        public async Task Handle(OrderExportedEvent message)
        {
            ...
            await _telegramService.SendMessageAsync($"ПЕРЕДАН В ПРОИЗВОДСТВО: заказ \"{message.Order.Title}\" пользователя: \"{message.Order.User}\".");
        }
    }
}