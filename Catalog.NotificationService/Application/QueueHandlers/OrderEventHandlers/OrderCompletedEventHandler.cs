...
namespace Catalog.NotificationService.Application.QueueHandlers.OrderEventHandlers
{
    public class OrderCompletedEventHandler : IHandleMessages<OrderCompletedEvent>
    {
        ...
        public OrderCompletedEventHandler(ITelegramService telegramService,
        ...
        public async Task Handle(OrderCompletedEvent message)
        {
            ...
            await _telegramService.SendMessageAsync($"ЗАКАЗ ЗАВЕРШЕН: заказ \"{message.Order.Title}\" пользователя: \"{message.Order.User}\".");
        }
    }
}